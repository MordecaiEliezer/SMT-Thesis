...
# Load models
# -----------------------------
# Get the folder where the script/exe is running
if getattr(sys, 'frozen', False):
    # Running as compiled exe
    app_path = sys._MEIPASS  # PyInstaller temp folder for bundled files
    exe_dir = os.path.dirname(sys.executable)
else:
    # Running as script
    exe_dir = os.path.dirname(os.path.abspath(__file__))

# Use relative paths to models
baseline_model_filename = os.path.join(exe_dir, "Baseline_fi_hi_translation_model.pkl")
proposed_model_filename = os.path.join(exe_dir, "Proposed_fi_hi_translation_model.pkl")
...
root.geometry("1400x600")  # Wider window to fit 3-column layout
...
center_frame.grid(row=0, column=1, padx=50, pady=20, sticky="n")

# Output Frame
output_frame = tk.Frame(root)
output_frame.grid(row=0, column=2, padx=30, pady=20, sticky="n")
...
text_input = tk.Text(input_frame, width=55, height=23)
...
baseline_output = tk.Text(output_frame, height=10, width=55, wrap="word", state=tk.DISABLED)
...
proposed_output = tk.Text(output_frame, height=10, width=55, wrap="word", state=tk.DISABLED)
